...
profile["default"] = profile.standart;
generator["default"] = generator.standart;
...
function restore_blocks_standart() {
    if ('localStorage' in window && window.localStorage.standart) {
        var xml = Blockly.Xml.textToDom(window.localStorage.standart);
...
function backup_blocks_standart() {
    if ('localStorage' in window) {
      var xml = Blockly.Xml.workspaceToDom(Blockly.mainWorkspace);
      window.localStorage.setItem('standart', Blockly.Xml.domToText(xml));
    }
  }

function auto_save_and_restore_blocks_standart() {
    // Restore saved blocks in a separate thread so that subsequent
    // initialization is not affected from a failed load.
    window.setTimeout(restore_blocks_standart, 0);
    // Hook a save function onto unload.
    bindEvent(window, 'unload', backup_blocks_standart);
    tabClick(selected);
