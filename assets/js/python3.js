...
profile["default"] = profile.rpi;
// generator["default"] = generator.python;
...
function showCodePython() {
    ...
    codePython.innerHTML = "<textarea class=\"lined\" readonly>" + code + "</textarea>";
    $(".lined").linedtextarea();
}
...
function restore_blocks_python() {
    if ('localStorage' in window && window.localStorage.python) {
        var xml = Blockly.Xml.textToDom(window.localStorage.python);
...
function backup_blocks_python() {
    if ('localStorage' in window) {
      var xml = Blockly.Xml.workspaceToDom(Blockly.mainWorkspace);
      window.localStorage.setItem('python', Blockly.Xml.domToText(xml));
    }
  }

function auto_save_and_restore_blocks_python() {
    // Restore saved blocks in a separate thread so that subsequent
    // initialization is not affected from a failed load.
    window.setTimeout(restore_blocks_python, 0);
    // Hook a save function onto unload.
    bindEvent(window, 'unload', backup_blocks_python);
    tabClick(selected);
...
function updateFunction(event) {
    ...
    l_lenght = l_lenght * 8;
    $('.code-python-IDE').val(code);
    document.getElementById('code-py').style.width = l_lenght + 'px';
    autosize($('.code-python-IDE'));
    ta = document.getElementById('code-py');
